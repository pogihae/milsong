...
export default async function ResultPage({ searchParams }: ResultPageProps) {
  ...
  const mainArt = result.candidates[0]?.albumArtUrl ?? null;
  const candidateArts = result.candidates.map((c) => c.albumArtUrl ?? null);
      ...
      <section className="mx-auto max-w-2xl space-y-5">
        ...
        {result.staleMode ? (
          <div className="flex items-start gap-2 rounded-xl border border-amber-500/25 bg-amber-500/8 px-4 py-3 text-sm font-medium text-amber-400">
            ...
            해당 시기 메가 히트곡이 뚜렷하지 않아 발매일이 가장 가까운 곡 기준으로 추천했어요.
          </div>
        ) : null}
