...
export function parseBugsDailyChart(html: string): DailyChartSnapshot {
  ...
  const rows: BugsChartRow[] = $('tr[rowtype="track"], tr[rowType="track"]')
    ...
    .map((element) => {
      ...
      const title = normalizeText(row.find('p.title a').first().attr('title') ?? row.find('p.title a').first().text());
      const artist = normalizeText(row.find('p.artist a').first().attr('title') ?? row.find('p.artist a').first().text());
      const albumTitle = normalizeText(row.find('a.album').first().attr('title') ?? row.find('a.album').first().text());
      ...
      if (!Number.isFinite(rank) || !title || !artist || !sourceSongId) {
