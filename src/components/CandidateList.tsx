...
function buildTags(breakdown: Candidate['breakdown']): string[] {
  const tags: string[] = [];
  const days = Math.round(breakdown.exposure);
  if (days > 0) tags.push(`이병 기간 TOP10 ${days}일`);
  if (breakdown.wins !== null && breakdown.wins > 0) tags.push(`음방 ${breakdown.wins}관왕`);
  return tags;
}
  ...
  return (
    <section className="glass-panel relative rounded-[2rem] p-6 sm:p-8">
      ...
      <ol className="space-y-4">
        {candidates.map((c) => {
          const tags = buildTags(c.breakdown);
          return (
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-lg font-bold text-slate-900">
                    {c.title} <span className="ml-1 text-base font-medium text-slate-500">{c.artist}</span>
                  </p>
                  {tags.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {tags.map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-semibold text-indigo-600 backdrop-blur-sm"
                        >
                          {tag}
                        </span>
                      ))}
